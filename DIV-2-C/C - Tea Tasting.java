...
//                TreeMap<Integer,Long> map = new TreeMap<>();
                ...
                for (int i = 1; i < n; i++) {
                    prefix[i] = people[i]+prefix[i-1];
                }
                int[] can = new int[n+1];
                long[] rem = new long[n+1];
                for (int i = 0; i < n; i++) {
                    int teaQty = tea[i];
                    long prev = i==0?0:prefix[i-1];
                    if (tea[i]<=people[i]){
                        can[i]--;
                        rem[i]+=tea[i];
                        continue;
                    ...
                    while (r - l > 1) {
                        ...
                        if (prefix[m] - prev > teaQty) {
                            r = m;
                        }else {
                            l = m;
                        }
                    }
                    long temp = teaQty - (l < 0 ? 0 : prefix[l] - prev);
                    can[r]--;
                    rem[r] += temp;
                }
                int streak = 1;
...
//                out.println(Arrays.toString(rem));
                for (int i = 0; i < n; i++) {
                    streak+=can[i];
                    ans[i] = (long) people[i] *streak;
                    ans[i]+=rem[i];
                    streak++;
                }
                for (int i = 0; i < n; i++) {
