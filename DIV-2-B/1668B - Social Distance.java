...
//        int times = 1;
    ...
    public static void solve(FastReader sc) {
        ...
        for (int i = 0; i < n-1; i++) {
            chairsNeed+=arr[i];
            chairsNeed+= (have>=arr[i])?0 : arr[i]-have;
            have=arr[i];
        }
        chairsNeed+=n-1;
        if (chairsNeed+((2*arr[n-1])+1)<=m){
            out.println("YES");
        }
        else {
            long spaceInEdge = 0;
            if (n>1){
                ...
                spaceInEdge+= arr[0];
                spaceInEdge+=arr[n-2];
                long spaceWithoutLast = m-chairsNeed;
                spaceInEdge+= (spaceWithoutLast>=0)?spaceWithoutLast :0;


            }
            long needMore = (arr[n-1]*2)+1;


            if (needMore<=spaceInEdge){
                out.println("YES");
            }
            else {
                out.println("NO");
