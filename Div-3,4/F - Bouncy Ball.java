...
//            int testcases = 1;
            while (testcases-- > 0) {
                ...
                int xAxis = 1;
                int yAxis = 1;
                if (move.charAt(0)=='U')xAxis = -1;
                if (move.charAt(1)=='L')yAxis = -1;
                vis=new HashSet<>();
                out.println(max(-1,dfs(n,m,x,y,a,b,xAxis,yAxis)));
    ...
    private static int dfs(int n, int m, int x, int y, int tarx, int tary,int xAxis,int yAxis){
        if(x==tarx && y == tary)return 0;
        String key=x+" "+y+" "+xAxis+" "+yAxis;
        if(vis.contains(key))return -100000;
        vis.add(key);
        int change = 0;
        if (x==1 && xAxis==-1){
            xAxis = 1;
       ...
       int add = change>0?1:0;

        x+=xAxis;
        y+=yAxis;

        return add + dfs(n,m,x,y,tarx,tary,xAxis,yAxis);
    }
